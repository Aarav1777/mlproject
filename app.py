...
def predict_datapoint():
    ...
    try:
        ...
        # convert to float if provided, otherwise use 0.0
        reading_score = float(reading_val) if reading_val not in (None, '', 'None') else 0.0
        writing_score = float(writing_val) if writing_val not in (None, '', 'None') else 0.0
        ...
        # Pass back the submitted values so template can re-populate the form
        return render_template(
            'home.html',
            results=final_score,
            gender=request.form.get('gender'),
        ...
        # return template with earlier inputs preserved (if any)
        return render_template(
            'home.html',
            results=error_msg,
            gender=request.form.get('gender'),
            ...
            lunch=request.form.get('lunch'),
            test_preparation_course=request.form.get('test_preparation_course'),
            reading_score=reading_val if 'reading_val' in locals() else '',
            writing_score=writing_val if 'writing_val' in locals() else ''
        )
